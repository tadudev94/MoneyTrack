...
export const share = async (testData : unknown) => {
  try {
    const timestamp = Date.now();
    const safeFileName = `share-${timestamp}.json`
    ...
    const dirExists = await RNFetchBlob.fs.exists(
      Platform.OS === 'android' ? dirs.CacheDir : dirs.DocumentDir,
    );
    ...
    const uriExists = await RNFetchBlob.fs.exists(
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('Uri.getScheme()')) {
        ...
        try {
          const dirs = RNFetchBlob.fs.dirs;
          const dirContents = await RNFetchBlob.fs.ls(
            Platform.OS === 'android' ? dirs.CacheDir : dirs.DocumentDir,
          );
